 ...
 *
 * This component replicates the `payment-failed-tap-to-pay.png` mockup using the Compost design system.
 */

import { X, X as XIcon, WalletCards, RotateCcw } from 'lucide-react'
    ...
    <div className="min-h-full bg-layer-level-0 text-foreground relative">
      ...
      <div className="px-6 pt-24 flex flex-col items-center text-center">
        <div className="relative mb-10">
          <div className="h-28 w-28 rounded-[9999px] bg-destructive/15 flex items-center justify-center">
            <div className="h-16 w-16 rounded-[9999px] bg-destructive flex items-center justify-center">
              <XIcon className="h-9 w-9 text-primary-foreground" />
            </div>
          </div>
        </div>

        <h1 className="text-[34px] font-semibold tracking-tight">Payment failed</h1>
        <div className="mt-2 text-[16px] text-muted-foreground">
          Transaction {transactionId} • {orderName}
        </div>
        <div className="mt-3 text-[16px] font-medium text-destructive">{failureReason}</div>

        <div className="mt-8 text-[54px] font-semibold tracking-tight">{formatMoney(amount)}</div>
      </div>
      ...
      <div className="fixed left-0 right-0 bottom-0 px-5 pb-6 pt-4 space-y-3 bg-layer-level-0">
        ...
        <Button type="button" className="h-12 w-full rounded-[12px]" onClick={onRetryPayment}>
