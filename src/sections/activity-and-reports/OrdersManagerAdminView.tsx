/**
 * OrdersManagerAdminView - Replicated design
 *
 * Replicates the "orders-manager-admin-view" mock using the Compost design system.
 */

import { useMemo } from 'react'
import { Printer, RotateCcw, Share2, ChevronDown, ChevronUp } from 'lucide-react'
...
export interface OrdersManagerAdminViewProps {
...
export default function OrdersManagerAdminView({
  data,
  activeTab = 'orders',
  ...
  onPrint,
  onRefund,
}: OrdersManagerAdminViewProps) {
  const defaults = useMemo(() => getDefaultActivityReportsData(), [])
  const orders = data?.orders ?? defaults.orders ?? []
  const expanded = expandedOrderId ?? orders.find((o) => o.items?.length && !o.isRefunded)?.id ?? orders[1]?.id ?? orders[0]?.id ?? ''
...
function OrderCard({
  ...
  const showActions = expanded && !order.isRefunded
        ...
        <div className="px-4 pb-4">
          ...
          {showActions ? (
            <div className="mt-5 grid grid-cols-2 gap-3">
              <Button type="button" variant="secondary" onClick={onPrint} className="h-14 rounded-[12px] gap-2">
              ...
              <Button
                ...
                variant="secondary"
                onClick={onRefund}
                className="h-14 rounded-[12px] gap-2 border border-destructive/40 bg-destructive/15 text-destructive hover:bg-destructive/20"
              >
