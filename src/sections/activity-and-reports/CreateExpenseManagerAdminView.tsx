/**
 * CreateExpenseManagerAdminView - Replicated design
 *
 * Replicates the "create-expense-manager-admin-view" mock using the Compost design system.
...
export interface CreateExpenseManagerAdminViewProps {
  initialDateLabel?: string
  initialName?: string
  initialAmount?: string
  initialTax?: '0%' | '10%' | '21%'
  onClose?: () => void
  onSave?: (payload: { dateLabel: string; name: string; amount: string; tax: '0%' | '10%' | '21%'; note: string }) => void
}

const KEYS = ['1', '2', '3', '4', '5', '6', '7', '8', '9', '.', '0', '⌫'] as const

export default function CreateExpenseManagerAdminView({
  initialDateLabel = 'October 24, 2023',
  initialName = '',
  initialAmount = '0.00',
  initialTax = '0%',
  onClose,
  onSave,
}: CreateExpenseManagerAdminViewProps) {
  const [dateLabel] = useState(initialDateLabel)
  ...
  const handleSave = () => {
    onSave?.({ dateLabel, name, amount, tax, note })
  }
      ...
      <div className="rounded-[18px] bg-card border border-border shadow-lg overflow-hidden">
        ...
        <div className="px-5 pt-4 pb-6">
          <div className="flex items-start justify-between gap-4">
            <div className="text-[22px] leading-[30px] font-semibold tracking-tight">Create expense</div>
            <Button
              type="button"
              variant="secondary"
              size="icon"
          ...
          <div className="mt-6 space-y-5">
            <Field label="Date">
              <Button type="button" variant="secondary" className="w-full h-12 rounded-[12px] justify-start gap-3">
                <Calendar className="h-4 w-4" aria-hidden="true" />
                <span className="text-small font-semibold">{dateLabel}</span>
              </Button>
            </Field>
